...
/**
 * Servlet implementation class EspaceInduServlet
 */
@WebServlet("/EspaceInduServlet")
public class EspaceInduServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EspaceInduServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String nomIndu;
		synchronized(this){
		nomIndu = (String)session.getAttribute("adresseMI");
		}
		String nomB;
		Double prix;
		String date;
		...
		try {
		    ...
		    try {
		      ...
		      ResultSet rs = stmt.executeQuery("select * from commerce WHERE nom_i = '" + nomIndu + "'");
		      while (rs.next()) {
		...
		synchronized(this){
		session.setAttribute("LCommerceI", listeCommerce.getVecCommerce());
		}
		request.getRequestDispatcher("VueEspaceI.jsp").forward(request, response);
	}
